...
<div class="container" style="padding-top: 3rem; padding-bottom: 5rem; max-width: 960px;">
  <div class="section-header" style="margin-bottom: 2.5rem;">
    ...
    <p>Get matched with a PhD specialist in your subject. 100% confidential, secure checkout, Turnitin report included.
    </p>
  </div>

  <form id="studentSubmitForm" class="calc-card" style="padding: 2.5rem; background: #ffffff;"
    enctype="multipart/form-data">
    <!-- Step 1: Contact Details -->
    <h3
      style="border-bottom:1px solid var(--border-color); padding-bottom:0.8rem; margin-bottom:1.5rem; color:var(--primary);">
    ...
    <div class="grid-2" style="display:grid; grid-template-columns:1fr 1fr; gap:1.2rem; margin-bottom:1.2rem;">
      <div class="form-group">
        ...
        <input type="text" name="name" class="form-control" required
          value="<?php echo htmlspecialchars($user['name'] ?? ''); ?>" placeholder="e.g. Sarah Jenkins">
      </div>

      <div class="form-group">
        <label>University Email *</label>
        <input type="email" name="email" class="form-control" required
          value="<?php echo htmlspecialchars($user['email'] ?? ''); ?>" placeholder="sarah@university.edu">
      </div>
    </div>

    <div class="grid-4"
      style="display:grid; grid-template-columns:1.2fr 1fr 1fr 1.2fr; gap:1.2rem; margin-bottom:2rem;">
      <div class="form-group">
        <label>Phone / WhatsApp *</label>
        <input type="tel" name="phone" class="form-control" required
          value="<?php echo htmlspecialchars($user['phone'] ?? ''); ?>" placeholder="+1 (555) 000-0000">
      </div>
    ...
    <div class="form-group">
      ...
      <input type="text" name="title" class="form-control" required
        placeholder="e.g. Machine Learning Hyperparameter Optimization Paper">
    </div>
    ...
    <div class="grid-3" style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:1.2rem; margin-bottom:1.2rem;">
      ...
      <div class="form-group">
        ...
        <select name="deadline_hours" id="formDeadline" class="form-control">
          <option value="120" <?php echo ($getDeadline === 120) ? 'selected' : ''; ?>>5 Days (Standard)</option>
    ...
    <div class="grid-3" style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:1.2rem; margin-bottom:1.2rem;">
      <div class="form-group">
        ...
        <input type="number" name="word_count" id="formWords" class="form-control" value="<?php echo $getWords; ?>"
          min="250" step="250">
      </div>
    ...
    <div class="form-group">
      ...
      <textarea name="instructions" class="form-control" rows="4" required
        placeholder="Paste full prompt, rubrics, formatting rules, or special guidelines here..."></textarea>
    </div>

    <!-- Step 3: File Uploads (Supports Any Format) -->
    <h3
      style="border-bottom:1px solid var(--border-color); padding-bottom:0.8rem; margin-bottom:1.5rem; margin-top:2rem; color:var(--primary);">
      <i class="fa-solid fa-cloud-arrow-up"></i> 3. Upload Files (Any Format Accepted)
    </h3>

    <div class="form-group">
      <div
        style="border: 2px dashed var(--portal-border); padding: 2rem; border-radius: var(--radius-sm); text-align: center; background: #f8fafc;">
        <i class="fa-solid fa-cloud-arrow-up"
          style="font-size: 2.5rem; color: var(--primary); margin-bottom: 0.8rem;"></i>
        <p style="color:var(--text-main); font-weight:700; margin-bottom:0.3rem;">Drag & drop your files here, or click
          to browse</p>
        <p style="color:var(--text-muted); font-size:0.85rem; margin-bottom:1rem;">
          Accepts <strong>ANY</strong> format: PDF, DOC, DOCX, ZIP, RAR, PPT, XLS, XLSX, JPG, PNG, PY, JAVA, CPP, IPYNB,
          SQL, TXT, etc. (Multiple files supported)
        </p>
        <input type="file" name="assignment_files[]" id="assignment_files" class="form-control" multiple
          style="max-width:400px; margin: 0 auto;">
        <div id="fileListSummary" style="margin-top:1rem; font-size:0.88rem; color:var(--primary); font-weight:600;">
        </div>
      </div>
    </div>
    ...
    <div
      style="background: rgba(79, 70, 229, 0.06); border: 1px solid var(--portal-border); border-radius: var(--radius-md); padding: 1.5rem; margin-top: 2rem;">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:1rem;">
        <div>
          ...
          <small id="formRateDetails"
            style="display:block; color:var(--text-dim); margin-top:2px; font-size:0.8rem;"></small>
        </div>
      ...
      <div style="display:flex; gap:10px; margin-top:1.2rem; align-items:center;">
        <input type="text" name="discount_code" id="formCoupon" class="form-control"
          placeholder="Promo Code (e.g. ACE20)" value="<?php echo htmlspecialchars($getCoupon); ?>"
    ...
    <div style="margin-top:1.5rem; display:flex; align-items:center; gap:10px;">
      ...
      <label for="termsCheck" style="font-size:0.85rem; color:var(--text-muted); margin:0;">
        I agree to the <a href="/terms.php" style="color:var(--primary);">Terms of Service</a> and <a
          href="/privacy.php" style="color:var(--primary);">Privacy Policy</a>.
      </label>
    </div>
